/*
 * GGToolkit static site generator. Outputs /dist with bilingual (ES/EN) hub:
 * home, tools directory, one page per tool (linking to the real tool domain),
 * guides, about, plus sitemap/robots/manifest/OG. No runtime dependencies.
 */
const fs = require('fs');
...
function tracking(){
  ...
  if (cfg.ga4Id) o += `\n<script async src="https://www.googletagmanager.com/gtag/js?id=${esc(cfg.ga4Id)}"></script>\n<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}gtag('js',new Date());gtag('config','${esc(cfg.ga4Id)}')</script>`;
  return o;
}
...
${ld ? `<script type="application/ld+json">${JSON.stringify(ld)}</script>` : ''}${tracking()}
</head><body>`;
}
function header(lang, current) {
  const t = T[lang];
  const other = cfg.languages.filter((l) => l !== lang);
...
function footer(lang) {
  ...
  const navLinks = `<a href="${pTools(lang)}">${esc(t.ui.tools)}</a><a href="${pGuides(lang)}">${esc(t.ui.guides)}</a><a href="${pAbout(lang)}">${esc(t.ui.about)}</a>`;
...
<div><h4>${esc(t.footer.siteTitle)}</h4>${navLinks}</div>
</div>
...
<span class="cat">${CATS[tool.category].icon} ${esc(catName(lang, tool.category))}</span>
</a>`;
}
...
function homeHTML(lang) {
  ...
  const ld = { "@context":"https://schema.org","@type":"WebSite","name":"GGToolkit","url":SITE+pHome(lang),"inLanguage":t.hreflang,"description":t.meta.homeDescription };
  const featured = TOOLS.map((tool) => toolCard(lang, tool)).join('\n');
  const cats = Object.keys(CATS).map((k) =>
    `<a href="${pTools(lang)}#${k}">${CATS[k].icon} ${esc(catName(lang, k))}</a>`).join('');
  const guides = GUIDES.slice(0, 3).map((g) => guideCard(lang, g)).join('\n');
  return head(lang, t.meta.homeTitle, t.meta.homeDescription, pHome(lang), alts, ld)
    + header(lang, 'home')
    + `<main class="wrap">
<section class="hero"><h1>${esc(t.home.h1)}</h1><p>${t.home.intro}</p></section>
<section class="section"><h2>🚀 ${esc(t.home.featured)}</h2><div class="grid">${featured}</div></section>
<section class="section"><h2>🗂️ ${esc(t.home.byCategory)}</h2><div class="cats">${cats}</div></section>
<section class="section"><h2>📖 ${esc(t.home.latestGuides)}</h2>${guides}<p style="margin-top:10px"><a href="${pGuides(lang)}">${esc(t.ui.guides)} →</a></p></section>
</main>`
    + footer(lang);
}
function toolsIndexHTML(lang) {
  ...
  const byCat = Object.keys(CATS).map((k) => {
    ...
    return `<section class="section" id="${k}"><h2>${CATS[k].icon} ${esc(catName(lang, k))}</h2><div class="grid">${list.map((x)=>toolCard(lang,x)).join('\n')}</div></section>`;
  }).join('\n');
  return head(lang, t.meta.toolsTitle, t.meta.toolsDescription, pTools(lang), alts, null)
    + header(lang, 'tools')
    + `<main class="wrap"><div class="crumb"><a href="${pHome(lang)}">${esc(t.ui.backHome)}</a></div>
<section class="hero" style="padding:24px 0 0;text-align:left"><h1>${esc(t.ui.tools)}</h1></section>${byCat}</main>`
    + footer(lang);
}
function toolPageHTML(lang, tool) {
  ...
  const ld = { "@context":"https://schema.org","@type":"SoftwareApplication","name":tool.name,"url":tool.url,
    "applicationCategory":"GameApplication","operatingSystem":"Web","inLanguage":t.hreflang,
    "description":tool.tagline[lang],"offers":{"@type":"Offer","price":"0","priceCurrency":"USD"} };
  const tags = tool.games.map((g) => `<span class="tag">${esc(g)}</span>`).join('');
  return head(lang, `${tool.name} — ${tool.tagline[lang]} | GGToolkit`, tool.tagline[lang], pTool(lang, tool.slug), alts, ld)
    + header(lang, 'tools')
    + `<main class="wrap"><div class="crumb"><a href="${pTools(lang)}">${esc(t.ui.backToTools)}</a></div>
<div class="tool-hero"><span class="ic">${tool.icon}</span><div>
<h1>${esc(tool.name)}</h1>
<p class="muted">${esc(tool.tagline[lang])}</p>
<div class="tags">${tags}</div>
<div class="btn-row"><a class="btn primary" href="${tool.url}" target="_blank" rel="noopener">${esc(t.ui.openTool)} ↗</a></div>
</div></div>
<article class="article"><p>${tool.description[lang]}</p>
<p><a class="btn" href="${tool.url}" target="_blank" rel="noopener">${esc(t.ui.visit)} ${esc(tool.name)} ↗</a></p>
</article>
<section class="section"><h2>${esc(t.ui.relatedTools)}</h2><div class="grid">${related.map((x)=>toolCard(lang,x)).join('\n')}</div></section>
</main>`
    + footer(lang);
}
...
function guidesIndexHTML(lang) {
  ...
  return head(lang, t.meta.guidesTitle, t.meta.guidesDescription, pGuides(lang), alts, null)
    + header(lang, 'guides')
    + `<main class="wrap"><div class="crumb"><a href="${pHome(lang)}">${esc(t.ui.backHome)}</a></div>
<section class="hero" style="padding:24px 0 14px;text-align:left"><h1>${esc(t.ui.guides)}</h1></section>
${GUIDES.map((g)=>guideCard(lang,g)).join('\n')}</main>`
    + footer(lang);
}
function guidePageHTML(lang, g) {
  const t = T[lang];
  const alts = {}; cfg.languages.forEach((l) => alts[l] = pGuide(l, g));
  const ld = { "@context":"https://schema.org","@type":"Article","headline":g.title[lang],"inLanguage":t.hreflang,
    "datePublished":g.date,"description":g.description[lang],
    "author":{"@type":"Organization","name":"GGToolkit"},"publisher":{"@type":"Organization","name":"GGToolkit"} };
  return head(lang, `${g.title[lang]} | GGToolkit`, g.description[lang], pGuide(lang, g), alts, ld)
    + header(lang, 'guides')
    + `<main class="wrap"><div class="crumb"><a href="${pGuides(lang)}">${esc(t.ui.guides)}</a></div>
<article class="article"><h1>${esc(g.title[lang])}</h1>${g.body[lang]}
<p style="margin-top:24px"><a class="btn" href="${pTools(lang)}">${esc(t.ui.allTools)} →</a></p></article></main>`
    + footer(lang);
}
function aboutHTML(lang) {
  const t = T[lang];
  const alts = {}; cfg.languages.forEach((l) => alts[l] = pAbout(l));
  return head(lang, t.meta.aboutTitle + ' | GGToolkit', t.meta.aboutDescription, pAbout(lang), alts, null)
    + header(lang, 'about')
    + `<main class="wrap"><article class="article" style="padding-top:18px"><h1>${esc(t.about.h1)}</h1>${t.about.body}
<p><a href="mailto:${esc(cfg.contactEmail)}">${esc(cfg.contactEmail)}</a></p>
<p style="margin-top:20px"><a class="btn primary" href="${pTools(lang)}">${esc(t.ui.allTools)} →</a></p></article></main>`
    + footer(lang);
}
function rootHTML() {
...
function sitemap() {
  ...
  cfg.languages.forEach((l) => { const al={}; cfg.languages.forEach((x)=>al[x]=pHome(x)); add(pHome(l), al, '1.0', 'weekly'); });
  cfg.languages.forEach((l) => { const al={}; cfg.languages.forEach((x)=>al[x]=pTools(x)); add(pTools(l), al, '0.9', 'weekly'); });
  ...
  cfg.languages.forEach((l) => { const al={}; cfg.languages.forEach((x)=>al[x]=pGuides(x)); add(pGuides(l), al, '0.7', 'weekly'); });
  GUIDES.forEach((g) => cfg.languages.forEach((l) => { const al={}; cfg.languages.forEach((x)=>al[x]=pGuide(x,g)); add(pGuide(l,g), al, '0.6', 'monthly'); }));
  cfg.languages.forEach((l) => { const al={}; cfg.languages.forEach((x)=>al[x]=pAbout(x)); add(pAbout(l), al, '0.3', 'yearly'); });
...
cfg.languages.forEach((l) => {
  ...
  write(path.join(DIST, l, aboutSlug(l), 'index.html'), aboutHTML(l));
  console.log('  /' + l + '/ (+ tools, guides, about)');
});
...
console.log('Done — ' + TOOLS.length + ' tools, ' + GUIDES.length + ' guides, ' + cfg.languages.length + ' languages.');
